...
- **In-Class Exercise:** Perform Monte Carlo Integration for $\int^{1}_{0}x^{3}dx$ with a random vector of length 500 drawn from $U[0,1]$.
...
- Recall that we need to find some differentiable $y=h(x)$ such that $x\in[a,b]$ becomes $y\in[0,1]$ and $dy=h^{\prime}(x)dx$. Preferably we want $h(x)$ to be **linear**. Fortunately there are ready-to-use results:
...
sidebarPanel(
  ...
  radioButtons("by", "Base for Y-series", 
               c("2"=1, "3"=2, "5"=3, "7"=4, "11"=5, "47"=15), width = '300px'),
  numericInput("it", "i-th Observation", 1, min=1, max=300, step=1, width = '300px')
),
...
server = function(input,output) {
  output$GHalton = renderPlot({
    ...
    if(i>300) {i<-300} else if (i<1) {i<-1} else if (is.na(i)) {i<-1}
    hlt<-halton(300,15)
    ghlt<-ghalton(300,5)[,c(4,5)] #keep only column 4 and 5
    rnd<-matrix(runif(600,0,1),300,2)
    ...
    grid.arrange(prnd,blank,phlt,pghlt,
...
- **In-class Exercise:** Consider the integration problem
$$
...
the workflow is as follows:
    ...
    Step 3: **Form a grid**. For example, assume that we look at a 3-dimensional problem ($x$, $y$, and $z$) with 2 nodes. Then Step 2 gives us $v^{(i)}$ and $v^{(i)}$. But we need to apply it to apply it to all 3 dimensions. So we have a grid of $2^3=8$ points: $(v^{(i)},v^{(i)},v^{(i)})$, $(v^{(i)},v^{(i)},v^{(i)})$, $(v^{(i)},v^{(i)},v^{(i)})$, and so on.
...
- The advantage of Quasi Monte Carlo and Monte Carlo will stand out if we further increase the dimension:
    ...
    - How high can the dimension be? Think about a general equilibrium quantitative trade model:
        ...
        - If you are asked to compute the welfare of a country in the economy by a change in a specific trade cost, then you are going to change all of the endogenous variables through general equilibrium, meaning that you will be doing a high-dimensional integration!
...
- **In-class exercise:** Try out the following 5-dimensional integration
$$
