...
- Sounds familiar? Think about how integrals are defined by Riemann Sum:
$$
\int^{a}_{b}g(x)dx \equiv \lim_{\Delta x \rightarrow 0}\sum_{i=1}^{n}g(x_i)\Delta x
$$
...
$$
\int_{2}^{20}\frac{1}{1+x}dx=\ln7\approx1.9459
$$
...
server = function(input,output) {
  ...
  n.fr<-(input$s.fr)+1 #number of nodes, needs to be integer greater than 2.
...
myFwdRec<-function(funct, lb, ub, n, ...) { #use ... to pass other inputs needed by funct
...
myFwdRec(funct=exfunc, lb=0, ub=6, n=100) #True value is 12.

#Can also write functions in myFwdRec directly. Come in handy when funct is not too complicated.
myFwdRec(funct = function(x) {5-x}, lb=0, ub=6, n=100)

#Try a function with multiple inputs
exfunc<-function(x,y,z) {5*y-x+z}
myFwdRec(funct=exfunc, lb=0, ub=6, n=100, y=1, z=2) #True value is 24.

#Try a non-function object
t<-3
myFwdRec(funct=t, lb=0, ub=6, n=100) 

#Try infinite boundary
myFwdRec(funct=function(x) {1+x}, lb=0, ub=Inf, n=100)

#Try invalid interval number
myFwdRec(funct=function(x){1+x}, lb=0, ub=3, n=-1)
...
server = function(input,output) {
  ...
  n.mr<-(input$s.mr)+1 #number of nodes, needs to be integer greater than 2.
...
server = function(input,output) {
  ...
  n.tr<-(input$s.tr)+1 #number of nodes, needs to be integer greater than 2.
...
server = function(input,output) {
  ...
  n.simp<-(input$s.simp)+1 #number of nodes, needs to be integer greater than 2.
  x.simp<-seq(2,20,length.out=n.simp) #nodes x_i
  sp.simp<-x.simp[2]-x.simp[1] #Step
...
server = function(input,output) {
  ...
  for (i in 2:1001) {
    ...
    #Evaluate values for nodes and midpoints
    y<-intfun(x)
    y.mid<-intfun((x+s/2))
