...
**In-class Exercise:** Implement the Midpoint with R. Refer to the code for Forward Rectangular Rule. The key idea is that you need to figure out how to get all the $n$ midpoints. If possible, wrap it into a function.
...
while (cont<=Ntol) { 
  ...
  if (cont==1000) {
    print("Converged with err < tol for consecutively 1000 times.")
    break
    }
  
  if(n==10000) {
    print("Do not converge to true value after 5000 of iteration.")
    break
  }
...
- We cover 4 different Gaussian Quadrature Rules (and discuss 3 of them). Which rule to use? Depends on the type of interval $[a,b]$ and the form of weighting function $p(x)$. 
    ...
    Consider a function $g(x)=p(x)f(x)$ and the interval of integration $[a,b]$, we can approximate the integral $\int^{b}_{a}g(x)dx$ by $\sum_{i=1}^{n}\omega_{i}f(x_{i})$ using the following rules once the requirements on $p(x)$ and $[a,b]$ are met respectively
    1. **Gauss-Legendre:** $p(x)=1$ and $[a,b]=[-1,1]$. 
    2. **Gauss-Chebyshev:** $p(x)=(1-x^2)^{-\frac{1}{2}}$ and $[a,b]=[-1,1]$. 
    3. **Gauss-Hermite:** $p(x)=e^{-(x^{2})}$ and $[a,b]=(-\infty,\infty)$.
    4. **Gauss-Laguerre:** $p(x)=e^{-x}$ and $[a,b]=[0,\infty)$. 
...
- The transformation $y=h(x)$ are in general NOT unique. You might come up with a more complicated $h(x)$ but still convert the interval into the one you need. Nevertheless, no need to go for complication. Oftentimes a **linear transformation** is the most brainless way to serve our purpose. The following are frequently used linear transformations that convert an interval into $[-1,1]$:

    - For $x\in[a,b]$, we let $x=\frac{a+b}{2}+\frac{b-a}{2}y$ so $dx=\frac{b-a}{2}dy$ and $y\in[-1,1]$.
    
    - For $x\in(-\infty,\infty)$, we let $x=\frac{y}{1-y^2}$ so $dx=\frac{y^2+1}{(y^2-1)^2}dy$ and $y\in[-1,1]$.
    
    - For $x\in[a,\infty)$, we let $x=a+\frac{1+y}{1-y}$ so $dx=\frac{2}{(y-1)^2}dy$ and $y\in[-1,1]$.
    
    - For $x\in(-\infty,b]$, we let $x=b-\frac{1-y}{1+y}$ so $dx=\frac{2}{(1+y)^2}dy$ and $y\in[-1,1]$.
