...
-   Continuous Random Variable
       ...
       - The numbers are **probability density**, and the function that assigns a number to
       each event $f(x)$ is the **probability density function (pdf)**.
       - Cumulated Probability: 
       $\Pr(L\leq x \leq \hat{X})\equiv F(\hat{X}) = \int^{\hat{X}}_{L}f(x)dx$, which is increasing 
...
-   Problems:
       ...
       - Even if we can, it is nearly impossible to *replicate* as the probability to run into the
       same realization is 0.
...
-   Pseudorandomness:
       ...
       - Most notable examples are probably computer games:
         ...
         - But how are the "probabilities" pre-determined? Depends on games: some are determined by
         the time you spent on the title screen, some are determined upon loading into the stages, 
         some are determined upon you turned on your Playstation 5.
         - Thus we have the "Save & Load" and "Rebooting" black magics. Not just to avoid undesired 
         outcome, but also to exploit the game mechanics, e.g., control the items dropped from mobs.
...
-   Random Number Generators (RNGs):
       ...
       - Suppose that we need 12 random numbers and we set the seed to 1. What are the 11th and 
      12th numbers? Can you predict the 53th random number if we need more?
       - Various algorithms are developed to improve the quality of "randomness" while maintaining 
       reproducibility (and saving computational resources, too).
...
-   `sample()` draws a randomized sample of a given size out from an object. Specifically,
       ...
       - `size` is a **positive integer** that determines the number of elements 
       for the sample drawn.
...
sliderInput("mu", "Mean", 0, min = -10, max = 10)
sliderInput("sigma", "SD", 1, min = 0, max = 10)
...
renderPlot({
  ...
  den<-dnorm(qx,0,1)
  x.dom<-seq(-4,qx,0.025)
  y.pdf<-dnorm(x.dom,0,1)
  ...
  ppdf<-ggplot(data = data.frame(x = c(-4, 4)), aes(x)) +
    ...
    geom_ribbon(data=data.frame(x=x.dom,ymin=rep(0,length(x.dom)),ymax=y.pdf),
                aes(x=x.dom,ymin=ymin,ymax=y.pdf),fill="blue",alpha=0.2,inherit.aes = FALSE)+
...
sliderInput("a","Tail Index", 2, min = 0.001, max = 5)
...
renderPlot({
  ...
  ggplot(data=df)+
    geom_histogram(aes(x=x,y=..density..),
                   bins=100,alpha=0.5,fill="blue",color="grey",position="identity")+
    geom_function(fun=frechet.pdf, 
                  data=data.frame(seq(m,10,0.0025)),size=1.5)+xlim(c(m,10))+ylim(c(0,1.5))
})
```
