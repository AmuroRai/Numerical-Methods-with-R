...
$$
f^{\prime\prime}(x)\approx\frac{f^{\prime}(x+\frac{h}{2})-f^{\prime}(x-\frac{h}{2})}{h}=\frac{f(x+\frac{h}{2})-2f(x)+f(x-\frac{h}{2})}{h^2}
$$
...
- But we are working on a computer, on which *real numbers* are approximated by *float numbers*. Thus we have a precision issue. The **machine epsilon** on R (and most of the programming languages) is $2^{-52}$ (falls somewhere between $10^{-16}$ and $10^{-15}$).
    - Machine epsilon is a number, such that all numbers $x$ equal or less than it are treated as 0 in the sense that $1+x=1$. In a sloppy way, you can think that machine epsilon is too small to be meaningful hence can be treated as 0.
...
4. Nash (1990) proposes to use a location-dependent step size. Specifically, he suggests that we use $h=\epsilon\times(|x|+\epsilon)$ where $\epsilon$ is a small number. Repeat 2 and 3 using Nash's approach, and we compare diffferent settings of $\epsilon$ by trying out $\epsilon\in\{2^{-26},10^{-7},10^{-6},10^{-5}\}$.
...
$$
$$
\frac{\partial^{2}f(x,y)}{\partial x^{2}} \approx \frac{f(x+\frac{h}{2},y)-2f\left(x,y\right)+f(x-\frac{h}{2},y)}{h^{2}}
$$
...
$$
\frac{\partial^{2}f(x,y)}{\partial y\partial x}\approx\frac{f\left(x+\frac{h}{2},y+\frac{k}{2}\right)-f\left(x,y+\frac{k}{2}\right)-f\left(x+\frac{h}{2},y\right)+f\left(x,y\right)}{hk}
$$
