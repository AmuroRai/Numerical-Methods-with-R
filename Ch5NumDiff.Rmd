...
ggplot(data=rst,aes(x=log(Step),y=log(Error)))+
  geom_line(color="blue",size=1,alpha=0.7)+
  geom_point(aes(x=log(h)))+
  annotate(geom="text",x=log(rst$Step[-length(h)]),y=log(rst$Error[-length(h)]),
           label=as.character(h)[-length(h)],
           size=3,vjust=-1,hjust=1)+
  annotate(geom="text",x=log(h[length(h)]),y=log(rst$Error[length(h)]),
...
rst #Cube root of machine epsilon or above are ok.
