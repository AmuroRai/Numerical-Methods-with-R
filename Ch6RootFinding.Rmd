...
- The fixed point iteration, also known as contraction mapping iteration, is an implementation to Banach Fixed Point Theorem. 
    ...
    If $h(x)$ is a contraction mapping ($h^{\prime}(x)<0$), then the initial guess and updates are the sequences generated from this contraction mapping, and Banach Fixed Point Theorem guarantees that eventually we get the fixed point if we do enough of iterations.

- In short:
    1. One can convert a root finding problem into a fixed point problem.
    2. **Require $h^{\prime}(x)<0$ for fixed point iteration to work.** 
