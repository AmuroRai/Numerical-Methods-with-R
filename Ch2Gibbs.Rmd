...
- So what is the probability that a female student is going to have noodle for lunch?
    ...
    $$
    \Pr(Dinning Choice = Noodle|Gender = Female) = \frac{\Pr(Dinning Choice = Noodle, Gender = Female)}{\Pr(Gender = Female)} = \frac{0.35}{0.55}=\frac{7}{11}
    $$
    - In fact,
    $$
    \Pr(Dinning Choice = Rice|Gender = Female) = \frac{\Pr(Dinning Choice = Rice, Gender = Female)}{\Pr(Gender = Female)} = \frac{0.15}{0.55}=\frac{3}{11}
    $$
    $$
    \Pr(Dinning Choice = Burger|Gender = Female) = \frac{\Pr(Dinning Choice = Burger, Gender = Female)}{\Pr(Gender = Female)} = \frac{0.05}{0.55}=\frac{1}{11}
    $$
    $$
    \Pr(Choice = Noodle|Gender = Female)+\Pr(Choice = Rice|Gender = Female)+\Pr(Choice = Burger|Gender = Female)=1
    $$
...
Let's see it visually. Each point represents an observation we keep. The intermediate observations are not show, but can be seen from the path we travel.
```{r, echo=FALSE, message=FALSE}
shinyApp(
ui = fluidPage(
  sidebarLayout(
